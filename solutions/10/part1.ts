...
const findTrailScore = (grid: string[][], row: number, column: number, heights = new Set(), prev = ''): any => {
  const currentValue = +grid[row][column];

  if (currentValue === 9) {
    heights.add(row + ',' + column);
    return 1;
  }
  ...
  Object.values(directions)
    .map((dir) => {
      ...
      if (slope === 1 && prev !== (newPosition[0] + ',' + newPosition[1])) {
        return findTrailScore(grid, newPosition[0], newPosition[1], heights, row + ',' + column);
      }
